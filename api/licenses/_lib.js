...
export function botPhotoApiPath(botId) {
  return `/api/licenses/photo?botId=${encodeURIComponent(safePhotoId(botId))}`;
}
...
export async function persistBotPhoto(botId, photo) {
  ...
  if (value === "/logo.png" || value.startsWith("/api/licenses/photo")) return value;
  ...
  try {
    ...
    if (error.status !== 404) {
      // Fall back to leaving data URL out of the shared JSON — use logo rather than fail license.
  ...
  await mutateStore((licenses) => {
    ...
    if (existing) {
      ...
      const shouldReplacePhoto =
        Boolean(bot?.photo) &&
        bot.photo !== "/logo.png" &&
        (prevPhoto === "/logo.png" || !prevPhoto || prevPhoto.startsWith("data:"));
      result = {
        ...existing,
